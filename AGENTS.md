...
- 常用命令:`just qa`(静态门禁 + 全部测试 + golden + pending + schema;等价 CI,默认 L0 静默输出,`just QA_VERBOSE=2 qa` 全量排障)/ `just golden`(skills 渲染基线门,zh-Hans + en 双 locale)/ `just smoke-context`(真实 LLM 冒烟)/ `bun run build`(单二进制)
