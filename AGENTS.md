...
- 常用命令:`just qa`(静态门禁 + 全部测试 + skills 模板渲染门 + pending + schema;等价 CI,默认 L0 静默输出,`just QA_VERBOSE=2 qa` 全量排障;单独跑渲染门:`just check-skills-template-render`)/ `just smoke-context`(真实 LLM 冒烟)/ `bun run build`(单二进制)
