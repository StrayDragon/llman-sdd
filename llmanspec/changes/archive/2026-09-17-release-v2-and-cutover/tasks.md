...
- [x] T7 发布执行(外发,需用户显式指令) [blocked-by: T3, T5, T6]
  ...
  - 完成注记(2026-09-17):v0.1.1 发布(run 35178255125 全绿)后 CLI 包名定案 `@llman-sdd/cli` + 双 bin(`llman-sdd`/`llmanspec`);v0.1.2 为首个完整正确版本(run 35178686645 全绿:npm 双包 + 五平台二进制 + GitHub Release)。踩坑记录:①unpublish 后 24h 禁同版本重发 → 升版本号;②bun.lock 未同步导致 workspace 替换写死旧版本 → bump 后必须 bun install;③registry CDN 滞后导致幂等守卫误跳 → 撤+无条件重发;④Node 24 拒绝对 node_modules 内 TS 做 stripping → npm 包声明 bun 运行时,Node 用户走二进制。已知表面项:已发布 0.1.2 的 `--version` 回显 0.1.0(core 硬编码常量,已在分支改为读 package.json,下版生效)
