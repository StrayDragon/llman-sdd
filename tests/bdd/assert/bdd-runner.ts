...
// bdd.run_command 指向 bun test tests/bdd,bindings 绑定 executable 标签。
...
if (!/tags:\s*\[executable\]/u.test(config)) {
  fail('bdd bindings 未绑定 executable 标签(@executable 场景不可被发现)');
}
